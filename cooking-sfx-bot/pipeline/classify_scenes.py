...
- cutting: 包丁・ハサミで切る動作の瞬間
- mixing: 泡立て器・スプーンで混ぜる動作の瞬間
- pouring: 液体を注ぐ・粉を入れる瞬間
- intro: 食材の提示・冒頭
- plating: 盛り付け
...
ルール:
- 1区間は最大3秒以内にする。同じ動作が続く場合は複数区間に分割する
  例: 10秒間ずっと混ぜている → 3秒以内の区間を複数に分ける
- 動作の開始タイミングを正確に指定する
- confidenceを付ける（0.0-1.0）
- 動作が見えないフレームは分類しない（無理に埋めない）
