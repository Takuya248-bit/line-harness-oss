...
## 作業ログの管理（秘書の責務）
- 全エージェントが `notes/YYYY-MM-DD-progress.md` に作業ログを残すルール
- 秘書は日次レビューでログの漏れがないかチェックする
- ログが不十分な作業を発見した場合、該当エージェントに追記を指示する
- ログ形式: `.company/CLAUDE.md` の「作業ログルール」参照（正式フォーマット定義はそちらが一次情報）

## フォルダ構成
- `inbox/` - 未整理のクイックキャプチャ
- `todos/` - 日次タスク管理（1日1ファイル）
- `notes/` - 壁打ち・相談メモ・意思決定ログ・作業ログ（1トピック1ファイル）
